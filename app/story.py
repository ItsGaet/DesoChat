import streamlit as st
from st_pages import Page, Section, show_pages, add_page_title

add_page_title()

st.markdown(body="**Desotech** è un istituto di formazione all’avanguardia con sede in Italia, specializzato in applicazioni cloud-native. Il nostro obiettivo è fornire servizi e competenze di alto livello alle organizzazioni che desiderano costruire e mantenere le proprie infrastrutture cloud-native o formare il proprio team IT all’uso di queste tecnologie rivoluzionarie che rappresentano il futuro di ogni ambiente di lavoro.")
st.markdown(body="### Esperienza e Impegno")
st.markdown(body="Siamo coinvolti in tutto ciò che riguarda l’universo cloud-native da oltre un decennio e operiamo in Italia sotto il marchio Desotech da oltre 5 anni, sviluppando il nostro nome e il nostro valore come centro di consulenza e formazione d’eccellenza.")
st.markdown(body="### Servizi Offerti:")
st.markdown(body="#### - Formazione")
st.markdown(body="Offriamo corsi approfonditi su tecnologie cloud-native, con formatori certificati per garantire una preparazione completa del vostro team")
st.link_button("✨ Formazione", "https://www.deso.tech/formazione")
st.markdown(body="#### - Consulenze e Management di Infrastrutture")
st.markdown(body="Dallo sviluppo della strategia all’implementazione pratica, fino alla formazione approfondita e completa, ci impegniamo a garantire ai nostri clienti le risorse di cui hanno bisogno per avere successo nell’era del cloud computing. I servizi gestiti da Desotech sono progettati per adattarsi alle esigenze uniche della vostra organizzazione. Che abbiate bisogno di un supporto break/fix in orario lavorativo o di una gestione e un monitoraggio globale H24, i nostri professionisti Kubernetes certificati sono qui per voi. Nessuna limitazione, solo assistenza di altissimo livello.")
st.link_button("☁️ Consulenze", "https://www.deso.tech/consulenza/")

st.markdown(body="**Un saluto da Desotech!**")
st.link_button("ℹ️ Linkedin", "https://www.linkedin.com/company/desotech/mycompany/")
